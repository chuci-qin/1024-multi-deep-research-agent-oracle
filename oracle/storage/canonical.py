...
class ResearchDataEntry(HashableData):
    ...
    thinking_process: list[dict] | None = Field(None)
    website_visits: list[dict] | None = Field(None)
    reasoning_chain: list[dict] | None = Field(None)
...
class OracleResearchData(HashableData):
    ...
    # Reference to config
    oracle_config_cid: str | None = Field(None)
    oracle_config_hash: str | None = Field(None)
    ...
    provable_data: dict | None = Field(None)
...
class VerifiedData(BaseModel):
    ...
    data: OracleConfigData | OracleResearchData
    canonical_json: str
    sha256_hash: str
    verified: bool = Field(default=False)

    @classmethod
    def from_data(cls, data: OracleConfigData | OracleResearchData) -> "VerifiedData":
