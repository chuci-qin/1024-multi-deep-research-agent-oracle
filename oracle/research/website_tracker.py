...
class WebsiteVisit(BaseModel):
    ...
    visited_at: str = Field(
        ...
        description="ISO timestamp of visit"
    )
    content_snippet: str = Field(
        default="",
        description="Excerpt of relevant content from the page"
    )
    ...
    credibility_tier: CredibilityTier = Field(
        default=CredibilityTier.TIER_5,
        description="Credibility tier"
    )
    ...
    relevance_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Relevance to the research question (0-1)"
    )
    
    # Metadata
    agent_id: Optional[str] = Field(None, description="Agent that visited")
...
TIER_3_DOMAINS = {
    ...
    # Finance
    "investopedia.com", "forbes.com", "fortune.com",
...
class WebsiteTracker(BaseModel):
    ...
    visits: list[WebsiteVisit] = Field(
        default_factory=list,
    ...
    def _categorize_source(self, domain: str) -> SourceType:
        ...
        if any(x in domain_lower for x in [
            "bbc", "cnn", "nytimes", "wsj", "bloomberg",
            "theguardian", "ft.com", "washingtonpost"
        ]):
            return SourceType.MAJOR_NEWS
        
        # Financial
        if any(x in domain_lower for x in [
            "tradingview", "yahoo", "marketwatch", "seekingalpha"
        ]):
            return SourceType.FINANCIAL
        
        # Crypto
        if any(x in domain_lower for x in [
            "coindesk", "cointelegraph", "coingecko", "coinmarketcap",
            "theblock", "decrypt", "bitcoin"
        ]):
            return SourceType.CRYPTO
        
        # Social
        if any(x in domain_lower for x in [
            "twitter", "x.com", "reddit", "discord", "telegram"
        ]):
    ...
    def get_top_sources(
        ...
        filtered = [
            ...
            if v.credibility_score >= min_credibility
        ]
    ...
    def get_statistics(self) -> dict:
        ...
        # Unique domains
        domains = set(v.domain for v in self.visits if v.domain)
