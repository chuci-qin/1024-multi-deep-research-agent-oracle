...
"""
    ...
    def _categorize_url(self, url: str) -> SourceCategory:
        ...
        # Major news
        news_domains = ['reuters', 'apnews', 'bloomberg', 'bbc', 'cnn', 'nytimes', 
                       'wsj', 'ft.com', 'cnbc', 'forbes']
        if any(x in domain for x in news_domains):
            return SourceCategory.NEWS
        
        # Crypto/finance specific
        crypto_domains = ['coindesk', 'cointelegraph', 'coingecko', 'coinmarketcap',
                         'binance', 'coinbase', 'investopedia', 'tradingview']
