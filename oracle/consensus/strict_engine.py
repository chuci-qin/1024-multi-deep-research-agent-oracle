...
class StrictConsensusConfig(ConsensusConfig):
    ...
    min_tier1_sources: int = Field(
        default=2, description="Minimum Tier 1 (official/wire) sources required"
    )
    min_tier2_sources: int = Field(
        default=3, description="Minimum Tier 2 (major news) sources required"
    )
    require_source_diversity: bool = Field(
        default=True, description="Require sources from multiple categories"
    )
    min_source_categories: int = Field(
        default=3, description="Minimum number of different source categories"
    )

    # Confidence requirements
    min_individual_confidence: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Minimum confidence per agent for vote to count"
    )
    min_consensus_confidence: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum weighted average confidence for consensus"
    )

    # Verification settings
    require_cross_verification: bool = Field(
        default=True, description="Require key facts to be verified across agents"
    )
    min_cross_verified_facts: int = Field(
        default=3, description="Minimum facts verified by multiple agents"
    )

    # Human review triggers
    max_confidence_spread: float = Field(
        default=0.3, description="Max confidence spread before requiring review"
    )
...
class DisagreementAnalysis(BaseModel):
    ...
    review_reason: str | None = Field(None)
...
class StrictConsensusEngine(ConsensusEngine):
    ...
    def __init__(self, config: StrictConsensusConfig | None = None):
    ...
    def calculate_strict(
        ...
        thinking_recorders: list[ThinkingRecorder] | None = None,
        website_trackers: list[WebsiteTracker] | None = None,
    ...
    def _verify_sources(self, results: list[AgentResult]) -> VerificationResult:
        ...
        tier1_count = sum(
            1 for s in all_sources if s.category in tier1_categories or s.credibility_score >= 0.9
        )
        tier2_count = sum(
            1
        ...
        categories = list({s.category.value for s in all_sources})
    ...
    def _analyze_disagreement_detailed(
        ...
        self.analyze_disagreement(results)
        ...
        for outcome_value in outcome_dist:
